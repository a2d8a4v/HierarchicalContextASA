...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ] ; then
    ...
    for repeat in "${repeat_list[@]}"; do
        input_dirs+="$root_dir/HSDG.sent.${lrs[$repeat]}.reweight2.0.regression.linear.all_s.roberta.glove.pmi5.interviewer.cefrgraph_init/ "
    done
    echo $input_dirs
    python3 collect_summary.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_$pred_method
fi
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ] ; then
    ...
    lrs=(1e-05 8e-06 6e-06 4e-06 2e-06)
    for repeat in "${repeat_list[@]}"; do
        input_dirs+="$root_dir/HSDG.sent.${lrs[$repeat]}.reweight2.0.regression.linear.test_wdc2.roberta.glove.pmi5.interviewer.cefrgraph_init/ "
    done
    python3 collect_summary.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_$pred_method

    python3 collect_summary_for_cefr_groups.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_$pred_method
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ] ; then
    ...
    lrs=(1e-05 8e-06 6e-06 4e-06 2e-06)
    for repeat in "${repeat_list[@]}"; do
        input_dirs+="$root_dir/HSDG.sent.${lrs[$repeat]}.reweight2.0.regression.linear.hec_s.roberta.glove.pmi5.interviewer.cefrgraph_init/ "
    done
    python3 collect_summary.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_$pred_method
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ] ; then
    ...
    lrs=(1e-05 8e-06 6e-06 4e-06 2e-06)
    for repeat in "${repeat_list[@]}"; do
        input_dirs+="$root_dir/HSDG.sent.${lrs[$repeat]}.reweight2.0.regression.linear.sde_s.roberta.glove.pmi5.interviewer.cefrgraph_init/ "
    done
    python3 collect_summary.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_$pred_method
fi
