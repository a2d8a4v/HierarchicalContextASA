...
def ordinal_entropy_tightness(features, gt,  mask=None, debug=False):
    """
    ...
    if _distance.numel() == 0:
      _entropy = torch.zeros_like(u_value)
      _entropy = _entropy.to(gt.get_device())
    else:
      _max = torch.max(_weight, dim=0).values
      _min = torch.min(_weight, dim=0).values
...
def ordinal_entropy(features, gt):
    ...
    samples = random.sample(range(0, len(gt)), len(gt))  # random sample 100 features
