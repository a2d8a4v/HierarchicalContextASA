...
def gai_loss(pred, target, gmm, noise_var):
    ...
    # print('balancing_term: ', balancing_term)
...
class BMCLoss(_Loss):
    def __init__(self, init_noise_sigma, device):
        ...
        self.noise_sigma = torch.nn.Parameter(torch.tensor(init_noise_sigma).to(device))
        self.device = device

    def forward(self, pred, target):
        noise_var = self.noise_sigma ** 2
        noise_var = noise_var.to(self.device)
        target = target.to(self.device)
        pred = pred.to(self.device)
        loss = bmc_loss(pred, target, noise_var, self.device)
        return loss
...
def bmc_loss(pred, target, noise_var, device):
    ...
    nt = torch.arange(pred.shape[0])
    nt = nt.type(torch.FloatTensor)
    nt = nt.to(device)
