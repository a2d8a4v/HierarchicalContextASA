...
class sentEncoder(nn.Module):
    ...
    def forward(self, input_feats):
        ...
        input_sent_len = ((input_feats != 0).sum(dim=1)).int()  # [s_nodes, 1]
        enc_embed_input = self.embed(input_feats)  # [s_nodes, L, D]
        ...
        if self._hps.cuda:
            input_pos = input_pos.to(self._hps.device)
...
class posEncoder(nn.Module):
    def __init__(self, hps):
        ...
        # word embedding
        self.embed = nn.Embedding(hps.pos_vocab_size, embed_size)
...
class depEncoder(nn.Module):
    def __init__(self, hps):
        ...
        # word embedding
        self.embed = nn.Embedding(hps.dep_vocab_size, embed_size)
...
class gedEncoder(nn.Module):
    def __init__(self, hps):
        ...
        # word embedding
        self.embed = nn.Embedding(hps.ged_vocab_size, embed_size)
...
class sentLenEncoder(nn.Module):
    def __init__(self, hps, embed):
        """
        ...
        :param hps: 
                ...
                cuda: bool, use cuda or not
        """
        super(sentLenEncoder, self).__init__()
        ...
        self.cnn_proj = nn.Linear(300 + embed_size, embed_size)
    ...
    def forward(self, input_feats):
        ...
        enc_maxpool_output = [F.max_pool1d(x, x.size(2)).squeeze(2) for x in enc_conv_output]  # kernel_sizes * [s_nodes, Co=50]
        sent_embedding = torch.cat(enc_maxpool_output, 1)  # [s_nodes, 50 * 6]
        enc_conv_input = enc_conv_input.squeeze(1)  # [s_nodes, L, D]
        enc_sent_len_embed_with_conv = self.cnn_proj(torch.cat((enc_conv_input, sent_embedding.unsqueeze(1).repeat([1, enc_conv_input.shape[1], 1])), dim=2))
        return enc_sent_len_embed_with_conv   # [s_nodes, L, D]
