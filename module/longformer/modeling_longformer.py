...
class LongformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # appended
        self.auxiliary_inputs_dict = config.auxiliary_inputs_dict
        for obj, vocab_size in config.auxiliary_inputs_dict.items():
            embedding = nn.Embedding(vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
            embedding.weight.data.normal_(mean=0.0, std=config.initializer_range)
            setattr(self, obj + '_auxiliary_input', embedding)
    ...
    def forward(
        ...
        token_type_ids=None,
        position_ids=None,
        pos_tokens_ids: Optional[torch.Tensor] = None, # appended
        deprel_tokens_ids: Optional[torch.Tensor] = None, # appended
        mlm_tokens_ids: Optional[torch.Tensor] = None, # appended
        w_cefr_tokens_ids: Optional[torch.Tensor] = None, # appended
        ged_tokens_ids: Optional[torch.Tensor] = None, # appended
        inputs_embeds=None
    ):
        ...
        # types of extended inputs:
        for obj, _ in self.auxiliary_inputs_dict.items():
            # at pre-processing stage, -1 is used as padding idx for other tasks, but we change to 0 referred as 'X' as padding token
            # here, we mask -1 to 0
            obtain_embed = vars()['{}_tokens_ids'.format(obj)]
            proced_embed = torch.where(obtain_embed < 0, 0, obtain_embed)
            embeddings = embeddings + getattr(self, obj + '_auxiliary_input')(proced_embed)
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        pos_tokens_ids: torch.Tensor, # appended
        deprel_tokens_ids: torch.Tensor, # appended
        mlm_tokens_ids: torch.Tensor, # appended
        w_cefr_tokens_ids: torch.Tensor, # appended
        ged_tokens_ids: torch.Tensor, # appended
        ...
        if padding_len > 0:
            ...
            if pos_tokens_ids is not None:
                pos_tokens_ids = nn.functional.pad(pos_tokens_ids, (0, padding_len), value=pad_token_id)
            if deprel_tokens_ids is not None:
                deprel_tokens_ids = nn.functional.pad(deprel_tokens_ids, (0, padding_len), value=pad_token_id)
            if mlm_tokens_ids is not None:
                mlm_tokens_ids = nn.functional.pad(mlm_tokens_ids, (0, padding_len), value=pad_token_id)
            if w_cefr_tokens_ids is not None:
                w_cefr_tokens_ids = nn.functional.pad(w_cefr_tokens_ids, (0, padding_len), value=pad_token_id)
            if ged_tokens_ids is not None:
                ged_tokens_ids = nn.functional.pad(ged_tokens_ids, (0, padding_len), value=pad_token_id)
        ...
        return padding_len, input_ids, attention_mask, token_type_ids, position_ids, pos_tokens_ids, deprel_tokens_ids, mlm_tokens_ids, w_cefr_tokens_ids, ged_tokens_ids, inputs_embeds
    ...
    def forward(
        ...
        padding_len, input_ids, attention_mask, token_type_ids, position_ids, pos_tokens_ids, deprel_tokens_ids, mlm_tokens_ids, w_cefr_tokens_ids, ged_tokens_ids, inputs_embeds \
            = self._pad_to_window_size(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            pos_tokens_ids=pos_tokens_ids, # appended
            deprel_tokens_ids=deprel_tokens_ids, # appended
            mlm_tokens_ids=mlm_tokens_ids, # appended
            w_cefr_tokens_ids=w_cefr_tokens_ids, # appended
            ged_tokens_ids=ged_tokens_ids, # appended
...
class LongformerForMaskedLM(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        config.auxiliary_inputs_dict={} # debug
