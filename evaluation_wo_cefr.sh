...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ] ; then
    ...
    for repeat in "${repeat_list[@]}"; do
        input_dirs+="$root_dir/HSDG.sent.${lrs[$repeat]}.reweight2.0.regression.linear.hec_s.roberta.glove.pmi5.interviewer.cefrNone/ "
    done
    python3 collect_summary.py --input_dirs $input_dirs \
                               --output_dir $root_dir/0_wo_cefr_$pred_method
fi
